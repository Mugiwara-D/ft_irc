...
void	Server::cmdUser(Command_s cmd, Client& client)
{
	...
	sendMessageToClient(client.getSocket(), RPL::BOUNCE(client.getNickname(), "10", "10", "9"));
	sendMessageToClient(client.getSocket(), RPL::MOTDSTART(client.getNickname(), SERV_NAME));
	sendMessageToClient(client.getSocket(), RPL::MOTD(client.getNickname(), "Welcome to a 42 irc server"));
